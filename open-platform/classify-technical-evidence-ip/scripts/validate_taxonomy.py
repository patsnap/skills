...
def main():
    ...
    empty_ids = [line for line, row in enumerate(rows, 2) if not row.get("label_id", "").strip()]
    ...
    if empty_ids: errors.append(f"rows missing label_id: {empty_ids}")
    ...
    for line, row in enumerate(rows, 2):
        ...
        if status == "formal":
            ...
            path = row.get("label_path", "").strip()
            name = row.get("label_name", "").strip()
            if path and name and path.split(" > ")[-1] != name:
                warnings.append(f"row {line}: final label_path component differs from label_name")
