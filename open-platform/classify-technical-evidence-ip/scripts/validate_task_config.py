...
def main():
    ...
    if not isinstance(data, dict):
        ...
        authorization = data.get("external_data_authorization", "not_requested")
        if authorization not in {"not_requested", "not_authorized", "authorized", "not_needed"}:
            errors.append("external_data_authorization has an unsupported value")
        if data.get("mcp_policy") not in {None, "none", "selective"}:
            errors.append("mcp_policy must be none or selective")
