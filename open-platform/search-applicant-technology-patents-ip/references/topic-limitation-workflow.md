...
If the user asks for a full draft before confirmation, mark every boundary item as `Pending user confirmation`.
...
Before any section named `Search strategy`, `Technology topic keywords`, `Applicant expression`, or any formula block, output this package. Keep the headings and tables visible in the report. Do not compress them into prose.
...
- Confirmation status: `Confirmed`, `Pending user confirmation`, or `Agent-inferred`.
...
| Element id | Facet | Local-language terms | English terms | Lower concepts / variants | Field scope | Proximity rule | Formula role | Confirmation status |
|---|---|---|---|---|---|---|---|---|
...
| Search element matrix | Matrix exists with local-language/English terms, fields, proximity, formula role | Pass / Fail |  |
...
For broad battery topics such as solid-state batteries, explicitly separate at least:
...
The decomposition table is mandatory. A statement such as "solid-state battery topic preflight" is not enough to satisfy this gate.

A table titled only `Technology topic keywords` is not enough. It must be transformed into the mandatory package above before formulas are produced.

## 3. Build the Term Matrix

Include:

- Local-language terms relevant to the applicants, jurisdictions, and technology.
- English terms.
...
| Element id | Facet | Local-language terms | English terms | Lower concepts / variants | Field scope | Proximity rule | Confirmation status | Notes |
|---|---|---|---|---|---|---|---|---|
