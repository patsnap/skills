...
- Do not write the report's `Search strategy` or formula section until the topic workflow's Mandatory Topic Preflight Package is present and all gates are `Pass` or explicitly marked `Pending user confirmation` in full-draft mode.
...
```text
The name or abbreviation "[name]" may refer to several applicants. Confirm the intended search scope:
1. [Full legal entity A] ([industry/domain])
2. [Full legal entity B] ([industry/domain])
3. [Full legal entity C] ([industry/domain])
4. Another entity (provide the full legal name)
```

Historical group template:

```text
The name or abbreviation "[name]" spans a corporate-history change. Confirm which entities belong in scope:
1. [Current listed/core entity] (current entity and common search scope)
2. [Parent or successor group] (group formed after [year])
3. [Historical entity] (independent before [year]; patents may remain under this name)
4. [Research institute/subsidiary] (may file separately)
5. Another entity (provide details)
```
...
- For a large multinational or corporate group, first produce an expansion table covering the core/listed entity, relevant parent or group entities, major R&D entities, manufacturing subsidiaries, historical names, local-language and English names, and user-provided entities.
...
- For non-Latin or local-language applicant values, use straight double quotes when required by query syntax and preserve the authoritative spelling.
...
| Topic definition and boundary table | Present, confirmed or marked `Pending user confirmation`. |
...
| Search-element matrix | Present, with local-language/English terms, field scope, proximity rule, and confirmation status. |
...
1. A usable family member in the user's target jurisdiction or market.
2. `WO`, `US`, or `EP` when no target-market member is usable and one of those records best supports international review.
3. Another jurisdiction when it is the most text-complete or commercially relevant record available.

Use `CN` first only when China is the target market or the user explicitly requests a China-first view.
...
0. Step 0: Original request and execution mode.
1. Step 1: Applicant entity expansion table.
2. Step 2: Technology topic definition.
3. Step 3: Topic boundary confirmation table.
4. Step 4: Technology topic decomposition table.
5. Step 5: Search element matrix.
6. Step 6: Noise and exclusion boundary table.
7. Step 7: Gate status before search.
8. Step 8: Search strategy and formulas, only after all Step 0-7 gates are `Pass` or `Pending`.
...
| Topic boundary | Confirmed, or marked `Pending user confirmation` in full-draft mode |  |  |  |
