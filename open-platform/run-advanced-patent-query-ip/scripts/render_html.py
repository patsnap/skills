#!/usr/bin/env python3
"""Render a structured competitor patent Markdown report as safe HTML."""
import sys
import re
import copy
...
# Scientific, accessible report styles.
STYLE = """
...
.hero{background:#173f63;border:1px solid #173f63;
  border-radius:4px;padding:30px 34px;margin-bottom:28px;color:#fff;position:relative;overflow:hidden;}
.hero::before{content:none;}
.hero-dots{display:none;}
.hero-inner{position:relative;z-index:1;}
.hero-badge{display:inline-block;background:transparent;border:1px solid rgba(255,255,255,.55);
...
.hero-stat{background:transparent;border:1px solid rgba(255,255,255,.4);
  border-radius:3px;padding:12px 20px;text-align:center;min-width:90px;}
...
.cbar-fill{height:100%;border-radius:0;background:var(--accent);}
...
.patent-fig{flex-shrink:0;width:180px;background:#f0f4f8;
...
.lit-ai-box{margin-top:24px;background:#f3f6f8;
...
@media print{
  ...
  .section,.patent-card,.lit-card{box-shadow:none;break-inside:avoid;}
  a{color:inherit;text-decoration:underline;}
  button{display:none;}
  .lit-abstract-full{display:inline;}
}
"""
...
def legal_tag(status):
    s = status or ""
    if "granted" in s.lower():
        return f'<span class="patent-tag granted">{s}</span>'
    if "pending" in s.lower() or "published" in s.lower():
...
def parse_md(md_text):
    """Parse the localized report Markdown with a single-pass state machine."""
    ...
    lit_query = ""
    lit_total = 0
    lit_ai_summary = []  # Lines of an explicitly supplied AI summary block.

    # State: None, overview, summary, detail, literature, or lit_ai.
    section = None
    cur_company = None
    cur_branch = None
    cur_patent = None
    ...
    for line in lines:
        ...
        m = re.match(r'>\s*Generated:\s*(.+)', line)
        if m:
            gen_time = m.group(1).strip()
            continue

        # Section transitions.
        if re.match(r'^## Part [1-5]:', line):
        ...
        if section == "overview":
            ...
            m = re.match(r'-\s*\*\*Returned records:\*\*\s*(\d+)', line)
            if m: total = int(m.group(1)); continue
            m = re.match(r'\|\s*(.+?)\s*\|\s*(\d+)\s*\|', line)
            if m and m.group(1).strip() not in ("Company or applicant group", "---", ":---"):
                company_counts[m.group(1).strip()] = int(m.group(2))
            continue

        # Company technology summary section.
        if section == "summary":
            m = re.match(r'^### (.+?) \(\d+\s*records\)', line)
            if m:
                cur_company = m.group(1).strip()
                summaries[cur_company] = {"directions": [], "rep_techs": []}
                continue
            if cur_company:
                m = re.match(r'-\s*\*\*Leading observed topics:\*\*\s*(.+)', line)
                if m:
                    for part in re.split(r';\s*', m.group(1)):
                        part = part.strip()
                        nm = re.match(r'(.+?) \((\d+)\)', part)
                        if nm:
                    ...
                    continue
                m = re.match(r'-\s*\*\*Representative technical approach:\*\*\s*(.+)', line)
                if m:
                    summaries[cur_company]["rep_techs"].append(m.group(1).strip())
                    continue
            continue

        # Detailed patent section.
        if section == "detail":
            # Company heading, exactly three hash characters.
            m = re.match(r'^### (.+?) \(\d+\s*records\)', line)
            ...
            # Technology branch, exactly four hash characters.
            m = re.match(r'^#### (.+?) \(\d+\s*records\)', line)
            ...
            # Patent entry, exactly five hash characters.
            m = re.match(r'^##### \[(.+?)\]\((.+?)\)', line)
            ...
            if cur_patent:
                for key, label in [("title","Title"),("legal","Legal status"),("assignee","Original applicant"),
                                    ("apdt","Application date"),("pbdt","Publication date"),("problem","Technical problem"),
                                    ("approach","Technical approach"),("benefit","Technical benefit")]:
                    m = re.match(rf'-\s*\*\*{label}:\*\*\s*(.+)', line)
                    if m: cur_patent[key] = m.group(1).strip(); break
                fm = re.match(r'-\s*\*\*Abstract drawing:\*\*.*!\[.+?\]\((.+)\)', line)
                if fm: cur_patent["fig"] = fm.group(1)

        # Literature section.
        if section == "literature":
            m = re.match(r'-\s*\*\*Derived literature query:\*\*\s*`(.+)`', line)
            if m: lit_query = m.group(1).strip(); continue
            m = re.match(r'-\s*\*\*Matching literature records:\*\*\s*(\d+)', line)
            if m: lit_total = int(m.group(1)); continue
            # Optional AI-assisted synthesis supplied by an approved caller.
            if re.match(r'^### AI-assisted literature synthesis', line):
            ...
            if cur_lit:
                ...
                m = re.match(r'-\s*\*\*Publication:\*\*\s*(.+)', line)
                ...
                m = re.match(r'-\s*\*\*Citation context:\*\*\s*(.+)', line)
                if m: cur_lit["citation"] = m.group(1).strip(); continue
                m = re.match(r'-\s*\*\*Abstract:\*\*\s*(.+)', line)
                if m: cur_lit["abstract"] = m.group(1).strip(); continue

        # Optional approved AI synthesis section.
        if section == "lit_ai":
...
def company_color(name):
    """Return a stable, restrained accent without company-specific branding."""
    palette = ("#245b8a", "#4f6b58", "#735c3f", "#5c6370")
    index = sum(ord(char) for char in (name or "")) % len(palette)
    return palette[index], "company"


def _safe_url(value, allow_relative_image=False):
    """Return a safe HTTP(S) URL or a constrained report-image path."""
    raw = str(value or "").strip()
    if allow_relative_image and re.fullmatch(r"images/[A-Za-z0-9_.-]+", raw):
        return raw
...
def _sanitize(value, key=""):
    """Escape parsed Markdown data and reject unsafe links and image paths."""
    if isinstance(value, dict):
        return {item_key: _sanitize(item_value, item_key) for item_key, item_value in value.items()}
    if isinstance(value, list):
        return [_sanitize(item, key) for item in value]
    if isinstance(value, str):
        if key in {"url", "doi_url"}:
            value = _safe_url(value)
        elif key == "fig":
            value = _safe_url(value, allow_relative_image=True)
        return html_module.escape(value, quote=True)
    return value


def render_html(data):
    data = _sanitize(copy.deepcopy(data))
    title = data["title"] or "Patent analysis report"
    ...
    # Exclude only the explicit Other bucket; use detailed records for the displayed total.
    named_companies = {k: v for k, v in company_counts.items() if k != "Other"}
    ...
    lit_nav = ""
    if literature:
        lit_ai_nav = '<div class="nav-sub"><a href="#lit-ai-box">AI-assisted synthesis</a></div>' if lit_ai_summary else ""
        lit_nav = f'<a href="#sec4" class="nav-part">Related literature</a>{lit_ai_nav}<div class="nav-sub"><a href="#lit-detail">Literature records</a></div>'

    sidebar = f"""
<div id="sidebar">
  <div id="sidebar-logo"><h2>Technology intelligence report</h2><p>{title}</p></div>
  <nav>
    <a href="#sec1" class="nav-part">Patent overview</a>
    <a href="#sec2" class="nav-part">Technology summaries</a>
    <a href="#sec3" class="nav-part">Patent details</a>
    {sidebar_company_links}
    {lit_nav}
  </nav>
</div>"""
...
<div class="hero">
  ...
  <div class="hero-inner">
    <div class="hero-badge">Technology intelligence report</div>
    <h1>{title}</h1>
    <div class="hero-meta">Generated: {gen_time}</div>
    <div class="hero-stats">
      <div class="hero-stat"><div class="hero-stat-num">{real_total}</div><div class="hero-stat-label">Patent records</div></div>
      <div class="hero-stat"><div class="hero-stat-num">{num_companies}</div><div class="hero-stat-label">Company groups</div></div>
      <div class="hero-stat"><div class="hero-stat-num">{num_branches}</div><div class="hero-stat-label">Technical topics</div></div>
    </div>
  </div>
</div>"""
...
<div class="section" id="sec1">
  <div class="section-hd">
    <div class="section-title">Recently published patent overview</div>
  </div>
  <div class="section-bd">
    ...
    for cname, info in summaries.items():
        ...
        chips = "".join(
            f'<span class="direction-chip">{d["name"]}{" (" + d["count"] + ")" if d["count"] else ""}</span>'
...
<div class="summary-card">
  <div class="summary-card-hd">
    ...
    <span class="summary-cnt">{cnt} records</span>
  </div>
...
<div class="section" id="sec2">
  <div class="section-hd">
    <div class="section-title">Company technology summaries</div>
  </div>
  <div class="section-bd">
    ...
    for ci, (cname, branches) in enumerate(details.items()):
        ...
        for bi, (branch, patents) in enumerate(branches.items()):
            ...
            for p in patents:
                ...
                # Status label.
                legal = p.get("legal", "")
                if "granted" in legal.lower():
                    status_cls = "granted"
                elif "pending" in legal.lower() or "published" in legal.lower():
                    status_cls = "pending"
                else:
                    status_cls = "other"
                ...
                fields_html = ""
                for key, label, lbl_cls in [("problem","Technical problem","lbl-problem"),("approach","Technical approach","lbl-approach"),("benefit","Technical benefit","lbl-benefit")]:
                    if p.get(key):
                        fields_html += f'<div class="field"><div class="field-lbl {lbl_cls}">{label}</div><div class="field-val">{p[key]}</div></div>'

                fig_html = ""
                if p.get("fig"):
                    fig_html = f'<figure class="patent-fig"><img src="{p["fig"]}" alt="Patent drawing for {p["pn"]}" loading="lazy"/><figcaption class="fig-cap">Patent drawing</figcaption></figure>'
  ...
  <div class="patent-card-hd">
    ...
    <span class="patent-tag apdt">Applied {p['apdt']}</span>
    <span class="patent-tag pbdt">Published {p['pbdt']}</span>
  </div>
...
<div class="branch-group">
  <div class="branch-label" id="branch-{ci}-{bi}">{branch} ({len(patents)} records)</div>
  {cards}
</div>"""
...
<div class="company-section">
  <div class="company-section-title" id="company-{ci}">
    ...
    {cname}
    <span class="company-cnt-badge">{total_c} records</span>
  </div>
  {branch_html}
</div>"""
...
<div class="section" id="sec3">
  <div class="section-hd">
    <div class="section-title">Patent details</div>
  </div>
  <div class="section-bd">
    {detail_html}
  </div>
        ...
        for idx, lit in enumerate(literature, 1):
            ...
            if abstract:
                if len(abstract) <= 300:
                    ...
                    rest = abstract[300:]
                    abstract_html = f'<div class="lit-abstract">{preview}<span class="lit-abstract-full" id="abs-{idx}">{rest}</span><button type="button" class="lit-abstract-toggle" aria-controls="abs-{idx}" onclick="toggleAbs({idx})">Show more</button></div>'
            else:
                abstract_html = ""

            meta_items = ""
            if lit.get("doi_url"):
                meta_items += f'<a class="lit-doi" href="{lit["doi_url"]}" target="_blank" rel="noopener noreferrer">DOI: {lit["doi"]}</a>'
            elif lit.get("doi"):
...
<div class="lit-card" id="lit-{idx}">
  ...
  <div class="lit-meta">{meta_items}</div>
        ...
        # Optional AI-assisted synthesis block.
        ai_box_html = ""
        if lit_ai_summary:
            # Convert a constrained Markdown subset after all content is escaped.
            ai_html_lines = []
            for ln in lit_ai_summary.splitlines():
                # Convert numbered references before generic strong text.
  ...
  <h4>AI-assisted literature synthesis</h4>
...
<div class="section" id="sec4">
  <div class="section-hd">
    <div class="section-title">Related literature ({lit_total} matches; showing {len(literature)})</div>
  </div>
  <div class="section-bd">
    {ai_box_html}
...
{section3}
{section4}
<div class="footer">Technology intelligence report · {gen_time}</div>
</div>
<script>{JS}</script>
</body>
</html>"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: render_html.py <report.md>", file=sys.stderr)
        sys.exit(1)
